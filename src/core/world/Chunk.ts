...
export class Chunk {
  ...
  build(data: ChunkData, manager: ChunkManager) {
    // 建立道路平台網格
    ...
    data.objects.forEach((inst) => {
      ...
      switch (inst.type) {
        case 'ring':
          mesh = manager.acquireRing();
          break;
        case 'energyCore':
          mesh = manager.acquireCore();
          break;
        case 'obstacle':
          mesh = manager.acquireObstacle(inst.subtype);
          break;
        case 'boostPad':
          mesh = manager.acquireBoost();
          break;
        case 'shard':
  ...
  dispose() {
    // road meshes are directly in group, we'll clear everything
